...
const page = usePage();
const organisations = computed(() => page.props.organisations || []);
const filteredOrganisations = computed<Organisation[]>(() => {
    let result = (organisations.value || []) as Organisation[];

    if (props.filters.searchTerm) {
        const search = props.filters.searchTerm.toLowerCase();
        result = result.filter(organisation =>
            organisation.name?.toLowerCase().includes(search) ||
            organisation.cvr?.toLowerCase().includes(search)
        );
    }

    if (props.filters.danishOnly) {
        result = result.filter(organisation => organisation.country.toLowerCase() === 'dk');
    }
    ...
    if (activeCountries.length > 0 && activeCountries.length < props.filters.countries.length) {
        result = result.filter(organisation =>
            activeCountries.includes(organisation.country)
        );
    }
...
const tableColumns = computed<ColumnDef<Organisation>[]>(() => {
...
const deletingOrganisation = ref<number | null>(null);

const handleRowClick = (organisation: Organisation) => {
    router.visit(route('sales.organisations.show', organisation.id));
};

const deleteOrganisation = (id: number) => {
    deletingOrganisation.value = id;
    fetch(`/sales/organisations/${id}`, {
        method: 'DELETE',
        headers: {
                ...
                >
                    ...
                    <TableCell>
                        <Button size="sm" variant="destructive"
                            @click.stop="deleteOrganisation(row.original.id)" :disabled="deletingOrganisation === row.original.id"
                        >
                            <LoaderCircle v-if="deletingOrganisation === row.original.id" class="h-4 w-4 animate-spin" />
                            Delete
                        </Button>
                    </TableCell>
                </TableRow>
